...
import { ChevronLeft, ChevronRight, MapPin, Sparkles, Home } from 'lucide-react';
...
const Calendar: React.FC<CalendarProps> = ({ onNavigate }) => {
  ...
  // Calculate days remaining until an Islamic event
  const calculateDaysUntilEvent = (hijriMonth: number, hijriDay: number): number => {
    // Approximate conversion: Hijri year is ~354 days, Gregorian is ~365 days
    // This is a simplified calculation - in production, use a proper Hijri calendar library
    const today = new Date();
    const currentYear = today.getFullYear();
    
    // Approximate Hijri dates to Gregorian for 2025 (1446-1447 AH)
    // These are approximate dates and should be updated with actual conversion
    const eventDates: Record<string, Date> = {
      '9-1': new Date(2025, 2, 1),   // Ramadan starts ~March 1, 2025
      '10-1': new Date(2025, 2, 30), // Eid al-Fitr ~March 30, 2025
      '12-10': new Date(2025, 5, 6), // Eid al-Adha ~June 6, 2025
      '1-10': new Date(2025, 6, 6),  // Ashura ~July 6, 2025
      '7-27': new Date(2025, 0, 26), // Isra and Miraj ~Jan 26, 2025
    };
    ...
    // If event has passed this year, use next year's date
    ...
    <div className="p-6 bg-slate-50 min-h-screen pb-32">
      <header className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          {onNavigate && (
            <button onClick={() => onNavigate(AppSection.Home)} className="p-4 bg-emerald-700 rounded-2xl border border-emerald-600 shadow-sm active:scale-95 transition-all text-white hover:bg-emerald-800">
              <Home size={20} />
            </button>
          )}
      ...
      <div className="mt-12 space-y-5">
        ...
        <div className="grid grid-cols-1 gap-4">
            {ISLAMIC_EVENTS.map((event, idx) => {
                const daysRemaining = calculateDaysUntilEvent(event.month, event.day);
                ...
                <div key={idx} className="bg-white p-5 rounded-[2.5rem] border border-slate-100 flex items-center gap-6 shadow-sm hover:shadow-md transition-shadow group">
                    ...
                    <div className="flex items-center gap-3">
                        {isToday ? (
                            <div className="px-4 py-2 bg-amber-500 text-white rounded-full shadow-lg">
                            ...
                            <div className="px-4 py-2 bg-emerald-100 text-emerald-700 rounded-full">
                                <span className="text-xs font-black">{daysRemaining} {daysRemaining === 1 ? 'day' : 'days'}</span>
                            </div>
                        )}
                        <div className="text-emerald-100 group-hover:text-emerald-300 transition-colors">
